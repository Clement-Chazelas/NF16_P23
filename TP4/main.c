    ...
    while (choix != '7') {
        ...
        switch (choix) {
            case '1' : ;
                ...
                printf("Entrez le nom du fichier que vous voulez charger (Il doit etre dans le meme dossier que ce programme)\n");  //On demande le nom du fichier que l'on doit scanner
                scanf("%s", &nom_fichier);
                indexerFichier(index, nom_fichier);     //On indexe le fichier
                viderBuffer();

                break;

            case '2' : ;
                printf("=====Caracteristiques de l'index======\n"); //On affiche le nombre de mots total et le nombre de mots distincts
             ...
             case '3' :
                afficherIndex(index);   //On appelle la fonction afficherIndex
                break;

            case '4' :;

                char motRecherche[100];
                printf("Entrez le mot que vous voulez rechercher : ");  //On demande le mot que l'on doit rechercher
                scanf(" %s", &motRecherche);

                // Appel de la fonction de recherche
                T_Noeud* resultat = rechercherMot(index, motRecherche);

                if (resultat != NULL) {     //Si on a trouver le mot on va afficher toutes ses occurences
                    printf("%s : \n", resultat->mot);
            ...
            case '5' : ;
                ...
                printf("Entrez le nom du mot cherche : ");  //On demande le mot que l'on recherche
                scanf(" %s", &mot_cherche);
                afficherOccurrencesMot(index, mot_cherche); //Pn appelle la fonction afficherOccurencesMot
                viderBuffer();
                break;

            case '6' :
                char nom_nouv_fichier[100];
                printf("Entrez le nom du nouveau fichier : ");  //On demande le nom du nouveau fichier
                scanf(" %s", &nom_nouv_fichier);
                construireTexte(index, nom_nouv_fichier);   //On appelle la fonction construire texte
                viderBuffer();
                break;
